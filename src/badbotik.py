...
def get_balance():
    try:
        r = requests.post(f"{API_URL}balance", json={"token": API_TOKEN}, timeout=10, verify=False)
...
def pay_user(user_id, amount):
    try:
        r = requests.post(f"{API_URL}pay", json={"token": API_TOKEN, "user": user_id, "count": amount}, timeout=10, verify=False)
        if r.status_code == 200:
            data = r.json()
...
def get_history(count=5):
    try:
        r = requests.post(f"{API_URL}getHistory", json={"token": API_TOKEN, "count": count}, timeout=10, verify=False)
        if r.status_code == 200:
            data = r.json()
