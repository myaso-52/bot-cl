...
def send_console_log(text):
    """Изолированная отправка логов СТРОГО в чат 2000000003"""
...
for event in longpoll.listen():
    if event.type == VkBotEventType.MESSAGE_NEW:
        ...
        if msg_lower in ["💰 баланс", "баланс"]:
            ...
            # Если игрок хочет сделать ручной запрос пополнения (например: пополнить 100мм)
        ...
        # РАНГ 3-5: НАЗНАЧЕНИЕ МОДЕРАТОРОВ (Иерархическая проверка)
            ...
            target_id = parse_target(parts, 1 if is_reply else 2, message_obj)
            if target_id:
                # Ограничение иерархии: Гл.Админ(3) до ранга 2, Зам(4) до ранга 3, Владелец(5) до 5
        ...
        # РАНГ 5: ВЛАДЕЛЕЦ (Выдача монет, ЛС Подтверждения, Системные фиксы)
            ...
            # Команда Владельца: пополнить [ссылка] [сумма] или в реплае: пополнить [сумма]
            is_reply = bool(message_obj.get('reply_message') or (message_obj.get('fwd_messages')))
            target_id = parse_target(parts, 1, message_obj)
            ...
            # Общее красивое универсальное меню, выдающееся на все базовые команды вызова
